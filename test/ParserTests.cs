...
namespace System.CommandLine.Tests
{
	...
	public class ParserTests
	{
		...
		public void TestArgumentNameCollisionInCommands()
		{
			...
			ParserOptions options = new ParserOptions();
			Parser parser = new Parser(options);
			parser.AddNamedArgument<DayOfWeek>("day-of-week", "d", "The day of the week as a string");
			Parser subParser = parser.AddCommand("test", "t", string.Empty);
			subParser.AddNamedArgument<DayOfWeek>("day-of-week", "d", "The day of the week as a string");

			// Parses the command line arguments
			ParsingResults firstParsingResults = parser.Parse(new string[] { "test.exe", $"{options.ArgumentPrefix}day-of-week", "thursday", "test", $"{options.ArgumentPrefix}day-of-week", "wednesday" });
			ParsingResults secondParsingResults = parser.Parse(new string[] { "test.exe", $"{options.ArgumentAliasPrefix}d", "Tuesday", "t", $"{options.ArgumentAliasPrefix}d", "Saturday" });
		...
		public void TestComplexScenario()
		{
			...
			parsingResults = parser.Parse(new string[] { "dotnet", "add", "./test.csproj", "reference", "../foo/bar.csproj", "--framework", "netcoreapp3.1", "-h" });
			Assert.False(parsingResults.GetParsedValue<bool>("Help"));
			Assert.True(parsingResults.HasSubResults);
			Assert.Equal("add", parsingResults.SubResults.Command);
			Assert.Equal("./test.csproj", parsingResults.SubResults.GetParsedValue<string>("Project"));
			...
			Assert.Equal("netcoreapp3.1", parsingResults.SubResults.SubResults.GetParsedValue<string>("Framework"));
			...
			parsingResults = parser.Parse(new string[] { "dotnet", "run", "--project", "./test.csproj", "-f", "netcoreapp3.1", "-h" });
			Assert.False(parsingResults.GetParsedValue<bool>("Help"));
			Assert.True(parsingResults.HasSubResults);
			Assert.Equal("run", parsingResults.SubResults.Command);
			Assert.Equal("./test.csproj", parsingResults.SubResults.GetParsedValue<string>("Project"));
			Assert.Equal("netcoreapp3.1", parsingResults.SubResults.GetParsedValue<string>("Framework"));
			Assert.True(parsingResults.SubResults.GetParsedValue<bool>("Help"));
			Assert.False(parsingResults.SubResults.HasSubResults);
		}
